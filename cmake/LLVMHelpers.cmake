macro(run_llvm_config VARIABLE_NAME)
  ...
  if(LLVM_CONFIG_RETVAL)
    ...
    set(${VARIABLE_NAME}
        ${LLVM_CONFIG_VALUE}
        CACHE STRING "llvm-config's ${VARIABLE_NAME} value")
...
macro(remove_prefix_from_filepath PATH_VAR)
  if(DEFINED ${PATH_VAR} AND ${PATH_VAR})
    ...
    if(PATH_VAR_START STREQUAL CMAKE_SYSROOT)
      ...
      message(
        STATUS
          "########## ${${PATH_VAR}} could not match ${CMAKE_SYSROOT} to ${PATH_VAR_START}"
      )
    endif()
  endif()
endmacro()
...
macro(find_llvm_program_or_die OUTPUT_VAR PROG_NAME LOCATION DOCSTRING)
  find_program(
    ...
    NAMES "${PROG_NAME}${LLVM_BINARY_SUFFIX}${CMAKE_EXECUTABLE_SUFFIX}"
          ...
          # /usr/lib/llvm-21/bin, but only 'clang++', 'clang' and clang-21
          # symlink. Thus when looking only in the install prefix, we don't find
          # the clang++-21 symlink that is only in /usr/bin. So, look also for
          # the non-suffixed ones since we are searching in the install/config
          # dir.
...
macro(custom_try_compile_c_cxx COMPILER SUFFIX SOURCE1 SOURCE2 RES_VAR)
  ...
  custom_try_compile_any(FALSE "${COMPILER}" ${SUFFIX} "${SOURCE_PROG}"
                         ${RES_VAR} ${ARGN})
endmacro()
...
macro(custom_try_compile_c_cxx_silent COMPILER SUFFIX SOURCE1 SOURCE2 RES_VAR)
  ...
  custom_try_compile_any(TRUE "${COMPILER}" ${SUFFIX} "${SOURCE_PROG}"
                         ${RES_VAR} ${ARGN})
endmacro()
